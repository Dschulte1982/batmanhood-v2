...
export const StockChart = (props) => {
  ...
  const showTooltipData = (data) => {
    if (data?.payload && typeof data?.payload[0] != "undefined") {
      return (
        <StockPrice
          first={stockData[0]?.closing}
          price={data.payload[0].payload.closing}
          name={props.name}
        />
      );
    }
  ...
  return !stockData || !props.stockPrice ? (
    <div className="spaceholder" />
  ) : (
    <div className="stock-chart">
      <div className="stock-price-container">
        <div className="stock-name">{props.name}</div>
        <div className="stock-price" id="current-price">
          {"$" + (props.stockPrice.toFixed(2) || "0")}
        </div>
      </div>
      <ResponsiveContainer width="100%" height={500}>
        <LineChart
          data={stockData}
          onMouseOver={hidePrice}
        ...
        >
          ...
          <Line
            stroke={
              props.stockPrice > stockData[0].closing ? "#03C805" : "#FF5103"
            }
            strokeWidth={1.8}
            yAxisId={0}
            dot={false}
