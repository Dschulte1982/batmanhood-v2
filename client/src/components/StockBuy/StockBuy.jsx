...
export const StockBuy = (props) => {
  ...
  return (
    <div className="buy-container">
      ...
      >
        ...
        <div className="stock-input">
          <div style={{ fontWeight: "bold" }}>
            {" "}
            {type === "Dollars" ? "Est. Quantity" : "Estimated Cost"}
          </div>
          <div>
            {type === "Dollars"
              ? (amount / Number(props.price) || 0).toFixed(2)
              : "$" + (amount * Number(props.price) || 0).toFixed(2)}
          </div>
        </div>
        <button type="submit" className="stock-buy-button" onClick={makeTrade}>
          Place Order
        </button>
      </form>
      ...
      <div className="balance">Your current balance is ${balance}</div>
    </div>
  );
};
