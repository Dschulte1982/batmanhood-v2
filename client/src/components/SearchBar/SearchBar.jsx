    ...
    if (e.target.value) {
      ...
      stockArray.forEach((ticker) => {
        for (const key in ticker) {
          ...
          if (
            (key === "Name" || key === "Symbol") &&
            ticker[key].toLowerCase().startsWith(checkStr.toLowerCase()) &&
      ...
      if (!window.location.href.includes("stocks") && i < 6) {
        pageData.push(
          <div className="search-ul">
            <div className="search-ul-1">
              <Link to={`/stocks/${array.Symbol}`}>{array.Symbol}</Link>
            </div>
            <div className="search-ul-2">
              <Link to={`/stocks/${array.Symbol}`}>{array.Name}</Link>
            </div>
          </div>
        );
        i++;
        ...
        pageData.push(
          <div className="search-ul">
            <div className="search-ul-1">
              <Link to={`${array.Symbol}`}>{array.Symbol}</Link>
            </div>
            <div className="search-ul-2">
              <Link to={`${array.Symbol}`}>{array.Name}</Link>
            </div>
          </div>
        );
        i++;
